      ...
      >
        {items.map((item, index) => (
          <li
            key={item.id}
            className={cn(
              "group relative cursor-pointer overflow-hidden rounded-lg border bg-card text-card-foreground shadow-xs",
              "md:min-w-[80px]",
              "min-h-0 min-w-0"
            )}
          ...
          >
            ...
            <article className="absolute inset-0 flex flex-col justify-end gap-2 p-4">
              ...
              <a
                href={item.linkHref}
                className="w-fit text-xs font-semibold text-white/90 underline underline-offset-4 opacity-0 transition-all duration-300 delay-300 group-data-[active=true]:opacity-100"
                onClick={(e) => {
                  if (activeIndex !== index) {
                    e.preventDefault();
                  }
